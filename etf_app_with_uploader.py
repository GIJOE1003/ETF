...
uploaded_file = st.file_uploader("Upload your model ZIP file", type=["zip"])
...
if uploaded_file is not None:
    ...
    if os.path.exists("rf_classifier.joblib") and os.path.exists("gbr_regressor.joblib"):
        rf_model = joblib.load("rf_classifier.joblib")
        gbr_model = joblib.load("gbr_regressor.joblib")
        st.success("Models loaded successfully!")
    else:
        st.error("Could not find the required model files in the ZIP. Expected: 'rf_classifier.joblib' and 'gbr_regressor.joblib'")
else:
    st.warning("Please upload the ZIP file containing the models to proceed.")
